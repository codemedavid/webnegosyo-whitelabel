...
This checkpoint supersedes the earlier “next slice” lists for setup, wallet, quote issuance, POS redemption, and destination syncing. The implementation is local and remains behind the existing release gates; this is not a deployment or pilot sign-off.

### Implemented in this continuation

- **Reward and rule setup on web and mobile:** fixed amount, capped percentage, and a selected free menu item; stamps or points, minimum spend, branch scope, reward expiry, optional Manila activation/end dates, pause/resume/end, and editing an existing program's reward/rules. Incomplete drafts can be repaired. Catalog ownership/availability and numeric rules are checked on the server. Program creation, initial version, revisions, and status transitions use a serialized SQL management contract; stale revisions are refused. Issued rewards keep their original terms.
- **Public customer wallet:** `/{tenant}/loyalty`, linked from checkout/order tracking and merchant setup. Phone-only lookup returns progress and reward terms, with durable tenant/phone/IP quotas and no profile, spend, address, or order history. The customer can request/resend the existing SMS OTP, verify it, and display the short-lived claim QR. Lost verification responses require a fresh code; verification is never automatically replayed.
- **Verified claim to POS:** authenticated server catalog/outlet pricing, one-use claim consumption, two-minute reward reservation, immutable quote, release, and expiry. Cancellation also records a tombstone when quote issuance is uncertain, preventing a delayed request from recreating a cancelled hold. Manual payment methods require a reference; methods requiring uploaded proof are excluded.
- **Merchant redemption screen:** scans or accepts a pasted QR, previews the reward/payable amount, collects supported tender, and shows the canonical receipt. The reservation ID is journaled before the quote request, and the exact sale ID/tender before settlement. Ordinary checkout checks both journals. Recovery survives tab focus, restart, and lost responses; account changes discard old screen state. New-sale navigation uses the app's safe tab helper. Existing receipts remain recoverable when the new-settlement gate is disabled.
- **Paid order projection:** atomic/idempotent platform Supabase, tenant Supabase, and Convex destinations, plus the scheduled outbox worker. Frozen line names/prices survive deleted catalog references. Receipt cash/change fields use the existing POS receipt shape. Customer capture and earning must finish before the job is acknowledged. Failed syncing is visible on web program management and can be explicitly rescheduled using the same settlement; the app links to that screen.
- **Settlement-time earning:** the canonical receipt freezes active program versions before commit. Delayed projection uses those terms even after a later pause or revision. This fixes historical selection for these canonical reward sales; the older general order-lifecycle/backfill path is not changed.
- **Full-refund reconciliation:** scheduled reads use authoritative destination payment-ledger evidence, never handset-provided status. Full refunds reverse the sale's earning and return the spent reward once, preserving expiry. Partial refunds do not return the reward. Zero-payable sales require cancellation/void/refund status. A reward whose originating earning was reversed is voided rather than made spendable again, with its retained threshold cost returned. Refund replay cannot restore a reward already reused on a later sale.
- **Maintenance:** expired reservations are released, old quota events and expired OTP records are removed in bounded batches, uncertain SMS dispatch records are retained, and paid receipts continue syncing even while new redemptions are disabled. `CRON_SECRET` authenticates the scheduled route.
...
- Real multi-session PostgreSQL contention, staging migrations, end-to-end browser/API testing, Android native-SIM delivery, enrollment/credential recovery on a real device, backend-outage exercises, and pilot monitoring/sign-off. No handset SMS or production deployment was performed.
- Inventory-tracked sales/modifiers, delivery/service charges, order-type overrides/markup, presell, bundles, linked modifiers, negative modifiers, and uploaded/provider-verified payment evidence need authoritative adapters. Quotes explicitly reject these cases. Voucher/manual-discount stacking remains excluded by the planned exclusive rule.
- Tenant Supabase installations without a payment ledger cannot prove positive-value refunds; restoration deliberately waits for evidence rather than trusting a status. Broader partial-refund earning recalculation and general order-lifecycle reconciliation remain separate work.
- Uncertain device enrollment still needs recovery without exhausting the device cap. Lost public verification responses safely require a new code rather than retrieving the consumed token.
- Opaque **membership** QR identification (distinct from the implemented verified **reward claim** QR), optional Semaphore delivery, and the broader unified Customer Hub/navigation/profile/aggregate work from this plan remain outstanding.

Final validation: **543 web tests across 43 suites**, **96 merchant-app tests across 12 suites**, and **all 12 isolated loyalty SQL runners** passed. These include rendered setup/wallet/POS recovery, cancellation races, exact settlement replay, destination deduplication, deleted catalog references, full-versus-partial refund evidence, refund replay/expiry, and settlement-time earning snapshots. Focused web ESLint and `git diff --check` passed; the Convex template prebundle regenerated successfully. Web and mobile TypeScript checks produced no diagnostics in the new production feature files, but repository-wide checks still fail on pre-existing test/type errors (including the existing loyalty progress test fixture and tutorial KitchenScene). No clean full build, real multi-session PostgreSQL, live handset, or staging deployment is claimed.
