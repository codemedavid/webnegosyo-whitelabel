...
describe('buildOrderTypePriceIndex', () => {
  ...
  it('coerces string numerics from the numeric column', () => {
    ...
    expect(buildOrderTypePriceIndex(rows)).toEqual({ 'ot-grab': { 'item-1': 150.5 } })
  })
  ...
  it('skips rows whose price is not a finite number', () => {
    ...
    expect(buildOrderTypePriceIndex(rows)).toEqual({ 'ot-grab': { 'item-3': 90 } })
  })
  ...
  it('lets the last duplicate win', () => {
    ...
    expect(buildOrderTypePriceIndex(rows)).toEqual({ 'ot-grab': { 'item-1': 175 } })
  })
...
describe('pricingForOrderType', () => {
  ...
  it('carries the markup and the item map for a known type', () => {
    ...
    ).toEqual({ orderTypeId: 'ot-grab', markupPercent: 25, itemPrices: { 'item-1': 150 } })
  })
...
describe('resolveItemPrice', () => {
  ...
  it('applies the markup to the list price', () => {
    expect(resolveItemPrice('item-1', 100, pricing({ markupPercent: 20 }))).toBe(120)
  })

  it('returns the list price when the markup is null', () => {
    expect(resolveItemPrice('item-1', 100, pricing({ markupPercent: null }))).toBe(100)
  })
