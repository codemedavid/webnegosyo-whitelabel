...
/**
 * Tokens Expo reports as belonging to a device that no longer exists. Tickets
 * come back positionally, one per message; anything that does not line up is
 * ignored rather than guessed at, so a malformed reply never deletes a live
 * device.
 */
export function collectStalePushTokens(
  messages: readonly { to: string }[],
  tickets: unknown
): string[] {
  if (!Array.isArray(tickets) || tickets.length !== messages.length) return []
  const stale: string[] = []
  tickets.forEach((ticket, index) => {
    if (typeof ticket !== 'object' || ticket === null) return
    const details = (ticket as { details?: { error?: unknown } }).details
    if (details?.error === 'DeviceNotRegistered') stale.push(messages[index].to)
  })
  return stale
}
