...
export async function POST(request: NextRequest): Promise<NextResponse> {
  if (process.env.LOYALTY_POS_SETTLEMENT_ENABLED !== 'true') {
    return respond({ error: 'Loyalty settlement is not available yet.' }, 503)
  }
  const authorization = request.headers.get('authorization')
  if (!authorization || !/^Bearer\s+\S+$/i.test(authorization)) return respond({ error: 'Unauthorized' }, 401)
  // Bound actual streamed bytes, not just the caller-controlled Content-Length.
  const raw = await readBody(request)
  if (raw instanceof NextResponse) return raw
  if (!isRecord(raw) || Object.keys(raw).some(key => !['tenantId', 'quoteId', 'clientOrderId', 'tender'].includes(key)) ||
    !isUuid(raw.tenantId) || !isUuid(raw.quoteId) || typeof raw.clientOrderId !== 'string' ||
    !/^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$/.test(raw.clientOrderId)) {
    return respond({ error: 'A tenant, quote, stable client order ID and tender are required.' }, 400)
  }
  const body = {
    tenantId: raw.tenantId.toLowerCase(), quoteId: raw.quoteId.toLowerCase(),
    clientOrderId: raw.clientOrderId, tender: raw.tender,
  }
  try {
    ...
    if (memberError || !member || !(member.role === 'superadmin' ||
      (member.role === 'admin' && member.tenant_id === body.tenantId)) ||
      !hasPermission(member, 'pos') || !hasPermission(member, 'loyalty_redeem')) {
    ...
    const { data: quote, error: quoteError } = await admin.from('loyalty_pos_quotes')
      .select('total_centavos, order_snapshot').eq('id', body.quoteId)
      .eq('tenant_id', body.tenantId).eq('created_by', user.id).maybeSingle()
    if (quoteError) return unavailable()
    if (!quote) return respond({ error: 'Quote not found.' }, 404)
    const policy: unknown = isRecord(quote.order_snapshot) ? quote.order_snapshot.paymentPolicy : null
    if (!isRecord(policy) || !Number.isSafeInteger(quote.total_centavos) ||
      policy.totalCentavos !== quote.total_centavos) {
      return invalidPolicy()
    }
    const payment = validateTender(policy, body.tender)
    if (!payment.ok && payment.error === 'invalid_policy') return invalidPolicy()
    if (!payment.ok) return respond({ error: payment.error }, 422)
    const { data, error } = await admin.rpc('settle_loyalty_pos_sale', {
      p_tenant_id: body.tenantId, p_quote_id: body.quoteId, p_client_order_id: body.clientOrderId,
      p_actor: user.id, p_payment: payment.value,
    })
...
function invalidPolicy(): NextResponse {
  return respond({ error: 'Quote payment policy is unavailable. Request a new quote.' }, 409)
}

/** Only known business errors may leave the database boundary. */
function settlementError(message: string): NextResponse {
  if (message === 'Forbidden branch' || message === 'Forbidden') return respond({ error: 'Forbidden' }, 403)
  const conflicts = [
    'Idempotency key belongs to a different request', 'Quote already settled', 'Quote expired',
    'Live loyalty is not enabled', 'Reward reservation expired or unavailable',
    'Reservation does not match quote', 'Reward is not valid at this branch',
  ]
  if (conflicts.includes(message)) return respond({ error: message }, 409)
  if (['Quote not found', 'Reservation not found', 'Reward not found'].includes(message)) {
...
function unavailable(): NextResponse {
  return respond({ error: 'Settlement could not be confirmed. Retry with the same client order ID.' }, 503)
}
