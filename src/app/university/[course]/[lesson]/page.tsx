    ...
    <div className="mx-auto max-w-6xl px-5 py-8 md:px-8 md:py-12">
      <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_320px]">
        <article className="min-w-0">
          <Link href={`/university/${course.slug}`} className="inline-flex items-center gap-1.5 text-sm font-semibold transition-opacity hover:opacity-70" style={{ color: SMARTMENU.cocoa }}>
          ...
          <header className="mt-5">
            ...
            {lesson.summary ? (
              <p className="font-serif mt-3 text-lg italic" style={{ color: SMARTMENU.cocoa }}>
                {lesson.summary}
              </p>
            ) : null}
            {lesson.durationMinutes ? (
              <p className="mt-3 inline-flex items-center gap-1.5 text-xs font-semibold" style={{ color: SMARTMENU.cocoa }}>
