...
import { CHECKOUT_URL, LANDING_COLORS, MARQUEE_ITEMS, PRICE_LABEL } from './landing-theme'
    ...
    <header className="fixed inset-x-0 top-0 z-50">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4 md:px-8">
        ...
        <div className="flex items-center gap-3">
          <a
            href="#pricing"
            className="hidden text-xs font-bold uppercase tracking-[0.12em] text-white/50 transition-colors hover:text-white md:block"
          >
            Pricing
          </a>
          <a
...
export function FeatureMarquee() {
  ...
  return (
    <div className="relative z-10 overflow-hidden border-y border-white/8 py-5" style={{ backgroundColor: 'rgba(6,4,3,0.85)' }}>
...
export function LandingFooter() {
  return (
    <footer className="relative z-10 border-t border-white/6 py-10 text-center" style={{ backgroundColor: LANDING_COLORS.ink }}>
      <span className="mx-auto mb-4 flex h-11 w-11 items-center justify-center overflow-hidden rounded-xl bg-[#f6f1e7]">
        <Image
          src="/webnegosyo-logo.png"
          alt="WebNegosyo logo"
          width={44}
      ...
      <p className="text-xs text-white/25">
        WebNegosyo • Smart Menu System • © {new Date().getFullYear()}
      </p>
    </footer>
  )
}
