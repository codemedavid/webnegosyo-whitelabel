...
import type { ModuleWithLessons } from '@/lib/university/service'

/** The done/total meter shared by the sidebar, the contents sheet and the course card. */
  ...
  return (
    <div>
      ...
      <div
        ...
        style={{ backgroundColor: `${SMARTMENU.ink}14` }}
        role="progressbar"
        aria-valuemin={0}
        aria-valuemax={total}
        aria-valuenow={done}
...
interface OutlineProps {
  ...
  completed: string[]
  /** Called after a lesson link is tapped, so an open sheet can close itself. */
  onNavigate?: () => void
}

/**
 * Every module and lesson of a course as a navigation list: the current lesson
 * highlighted, finished ones ticked. Rows clear 44px so they are comfortable
 * to hit with a thumb.
 */
export function CourseOutline({ courseSlug, modules, currentLessonSlug, completed, onNavigate }: OutlineProps) {
  return (
