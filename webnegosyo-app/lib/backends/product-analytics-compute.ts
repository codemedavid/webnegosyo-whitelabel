/**
 * Pure port of `convex-template/convex/productAnalyticsAggregator.ts`.
 *
 * Convex computes the BCG matrix in an action and STORES it in
 * `productAnalytics`; the app's refresh gesture re-runs that action. The
 * platform backend instead computes the same rows live, from the same three
 * inputs — non-cancelled orders, their line items, and the merchant's cost
 * prices — so "refresh" has nothing to write and the figures never go stale.
 *
 * Same thresholds, same medians, same rounding, same recommendation copy as
 * the Convex aggregator: a store moved between backends must not see its stars
 * reclassified.
 */
